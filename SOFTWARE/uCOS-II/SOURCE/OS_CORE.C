...
volatile    int         old_prio;
extern      char        msgBuffer1[40];
extern      char        msgBuffer2[40];
extern      OS_EVENT    *PrintBox;
/*$PAGE*/
/*
*********************************************************************************************************
*                                             INITIALIZATION
        ...
        if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Reschedule only if all ISRs complete ... */
            ...
            while(ptcb->OSTCBPrio != OS_IDLE_PRIO){
                if(ptcb->OSTCBStat == OS_STAT_RDY&& ptcb->OSTCBDly == 0){
                    if(earliest_deadline > ptcb->OSTCBDeadline || earliest_deadline == 0){
                        earliest_deadline = ptcb->OSTCBDeadline;
            ...
            if (!found) OSPrioHighRdy = OS_IDLE_PRIO;

            if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy */
                printf("\n%6d Preempt   ", OSTime);
                printf("%3d  %3d", old_prio, OSPrioHighRdy);
        ...
        while(ptcb->OSTCBPrio != OS_IDLE_PRIO){
            if((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY){
                if(earliest_deadline > ptcb->OSTCBDeadline || earliest_deadline == 0){
                    ...
                    OSPrioHighRdy = ptcb->OSTCBPrio;
                    found = 1;
                }else if( (earliest_deadline == ptcb->OSTCBDeadline) && OSPrioCur == ptcb->OSTCBPrio){
                    OSPrioHighRdy = ptcb->OSTCBPrio;
                    found = 1;
                }
            }
